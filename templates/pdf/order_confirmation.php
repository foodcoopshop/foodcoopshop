...
if (!empty($manufacturers)) {
    ...
    foreach ($manufacturers as $manufacturerId => $details) {
        ...
        $pdf->writeHTML('<h3>' . __('Order') . ': '. $appAuth->getUsername().', ' . __('placed_on') . ' '. $cart->modified->i18nFormat(Configure::read('app.timeHelper')->getI18Format('DateNTimeLong')).'</h3>', true, false, true, false, '');
        $pdf->Ln(8);
