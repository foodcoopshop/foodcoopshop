...
bin/cake i18n extract --output resources\\locales --paths config,src,templates --overwrite --extract-core no --merge no --no-location --exclude plugins
...
bin/cake i18n extract --plugin Admin --overwrite --extract-core no --merge yes --no-location
for locale in "${LOCALES[@]}"
do
...
bin/cake i18n extract --plugin Network --overwrite --extract-core no --merge yes --no-location
for locale in "${LOCALES[@]}"
do
