...
class SelfServiceController extends FrontendController
{
    ...
    public function index()
    {
        ...
        if (!empty($this->getRequest()->getQuery('keyword')) && count($products) == 1 && $keyword == $products[0]['ProductIdentifier']) {
            $this->CartProduct = TableRegistry::getTableLocator()->get('CartProducts');
            $result = $this->CartProduct->add($this->AppAuth, $products[0]['id_product'], 0, 1);
            if (!empty($result['msg'])) {
