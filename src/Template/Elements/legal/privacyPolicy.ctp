...
<p>1.1. Die Webseiten von <b>"<?php echo Configure::read('AppConfig.db_config_FCS_APP_NAME'); ?>"</b>, erreichbar unter der Domain <?php echo Configure::read('AppConfig.cakeServerName'); ?>,  werden von "
    <?php
    if (Configure::read('AppConfig.db_config_FCS_PLATFORM_OWNER') != '') {
        echo str_replace('<br />', ', ', Configure::read('AppConfig.db_config_FCS_PLATFORM_OWNER'));
    } else {
...
<p>2.1.1. Serverlogs: Die IP-Adresse des Anfragenden Computers, gemeinsam mit dem Datum, der Uhrzeit, der Anfrage, welche Datei angefragt wird (Name und URL), welche Datenmenge an Sie übertragen wird, eine Meldung, ob die Anfrage erfolgreich war, Erkennungsdaten des verwendeten Browsers und des verwendeten Betriebssystems, sowie die Website, von der der Zugriff erfolgte (sollte der Zugriff über einen Link erfolgen). Die Serverlogs werden gespeichert, um die Systemsicherheit prüfen zu können, die Website technisch administrieren, sowie das Angebot optimieren zu können. Diese Daten werden – sollte es einen Hackangriff gegeben haben – an die Strafverfolgungsbehörden weitergeben. Eine darüberhinausgehende Weitergabe an Dritte erfolgt nicht. Die Serverlogs werden für maximal drei Monate gespeichert.</p> 
