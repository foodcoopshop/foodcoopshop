...
<p>6.1. Sämtliche Leistungen werden von einem Guthabenkonto abgebucht. Das Guthabenkonto wird vom Betreiber verwaltet. Der Nutzer kann jederzeit auf das Guthabenkonto bis zu einem Maximalbetrag von <?php echo $this->MyHtml->formatAsEuro(Configure::read('appDb.FCS_PAYMENT_PRODUCT_MAXIMUM')); ?> Beträge einbezahlen.
...
if (Configure::read('appDb.FCS_MINIMAL_CREDIT_BALANCE') == 0) {
    ...
    echo 'Bis zu einem Kontostand von ' . $this->MyHtml->formatAsEuro(Configure::read('appDb.FCS_MINIMAL_CREDIT_BALANCE') * -1) . ' sind weitere Bestellungen möglich.';
}
?>
