    ...
    <div class="inner">
        ...
        if ($appAuth->user() && $this->Html->paymentIsCashless()) {
            ...
            echo '<div class="credit-balance-wrapper">';
              echo '<p><b><a href="'.$this->Slug->getMyCreditBalance().'">'.__('Your_credit_balance').'</a></b><b class="'.implode(' ', $class).'">'.$this->Html->formatAsCurrency($creditBalance).'</b></p>';
            if ($shoppingLimitReached) {
                echo '<p><b class="negative">'.__('You_reached_the_order_limit_{0}_please_add_credit.',[$this->Html->formatAsCurrency(Configure::read('appDb.FCS_MINIMAL_CREDIT_BALANCE'))]).'</b></p>';
        ...
        <?php if (!isset($shoppingLimitReached) || !$shoppingLimitReached) {  // set in AppController ?>
            ...
            <p class="products"></p>
            <p class="sum-wrapper"><b><?php echo __('Sum'); ?></b><span class="sum"><?php echo $this->Html->formatAsCurrency(0); ?></span></p>
            <p class="deposit-sum-wrapper"><b><?php echo __('Deposit'); ?></b><span class="sum"><?php echo $this->Html->formatAsCurrency(0); ?></span></p>
            <p class="tax-sum-wrapper"><b><?php echo __('Value_added_tax'); ?></b><span class="sum"><?php echo $this->Html->formatAsCurrency(0); ?></span></p>
