    ...
    public function main()
    {
        ...
        foreach ($customers as $customer) {
            ...
            if ($delta < 0) {
                $i ++;
                $deltaSum -= $delta;
                $delta = Configure::read('app.htmlHelper')->formatAsDecimal($delta) . ' ' . Configure::read('appDb.FCS_CURRENCY_SYMBOL'); // creditBalance is rendered in email view => do not use formatAsCurrency here because of &nbsp;
        ...
        $outString .= 'Summe: ' . Configure::read('app.htmlHelper')->formatAsCurrency($deltaSum * - 1) . '<br />';
