...
class CustomersFrontendControllerTest extends AppCakeTestCase
{
    ...
    public function testNewPasswordRequestWithValidEmail(): void
    {
        ...
        $customer = $customersTable->find('all',
            conditions: [
                $customersTable->aliasField('email') => Configure::read('test.loginEmailCustomer')
            ]
        )->first();
    ...
    public function testRegistrationWithCompanyUserActive(): void
    {
        ...
        $customer = $customersTable->find('all',
            conditions: [
                $customersTable->aliasField('email') => $email,
            ],
        )->first();
    ...
    public function testLoginPageWithOutputStringReplacements(): void
    {
        Configure::write('app.outputStringReplacements',
           include(APP . 'Services/OutputFilter/config/de_DE/memberClientConfig.php'),
        );
        $this->get($this->Slug->getLogin());
    ...
    private function saveAndCheckValidCustomer(array $data, string $email): Customer
    {
        ...
        $customer = $customersTable->find('all',
            conditions: [
                $customersTable->aliasField('email') => $customerAddressEmail,
            ],
            contain: [
                'AddressCustomers',
            ]
