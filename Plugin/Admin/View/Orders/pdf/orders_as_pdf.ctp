...
foreach ($orders as $order) {
    ...
    foreach ($order['OrderDetails'] as $orderDetail) {
        ...
        if ($i == count($order['OrderDetails'])) {
            ...
            $pdf->table .= '<td style="text-align: right"; width="' . $widths[3] . '"><h3>' . $this->Html->formatAsEuro($sumPrice) . '</h3></td>';
            if ($sumDeposit > 0) {
                $sumDeposit = $this->Html->formatAsEuro($sumDeposit);
            } else {
                $sumDeposit = '';
            }
            $pdf->table .= '<td style="text-align: right"; width="' . $widths[4] . '"><h3>' . $sumDeposit . '</h3></td>';
            $pdf->table .= '</tr>';
        }
