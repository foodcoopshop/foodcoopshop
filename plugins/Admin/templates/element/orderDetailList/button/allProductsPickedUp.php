...
if (count($pickupDay) == 1 && $groupBy == 'customer' && ($identity->isSuperadmin() || $identity->isAdmin() || $identity->isCustomer())) {
    ...
    echo '<button class="change-products-picked-up-all-customers-button ' . $class . '"><i class="fa-fw far fa-check-square"></i> ' . __('All_products_picked_up?') . '</button>';
}
...
if (count($pickupDay) == 1 && $groupBy == '' && $customerId > 0 && $manufacturerId == '' && $productId == '') {
    ...
    echo '<button class="change-products-picked-up-button ' . $class . '"><i class="fa-fw far fa-check-square"></i> ' . __('All_products_picked_up?') . '</button>';
}
