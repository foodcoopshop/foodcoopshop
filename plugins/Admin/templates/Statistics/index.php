...
<p><?php
    if (Configure::read('appDb.FCS_PURCHASE_PRICE_ENABLED')) {
        echo '<table class="list no-clone-last-row" style="width:350px;margin-bottom:5px;"><tr>';
            ...
            echo '<td>'. __('total') . '</td>';
            echo '<td><b>' . $this->Number->formatAsCurrency($totalNetTurnover) . '</b></td>';
            echo '</tr><tr>';
            echo '<td></td>';
            echo '<td>'. __('per_month') . '</td>';
            echo '<td>' . $this->Number->formatAsCurrency($averageTurnover + $averageNetProfit) . '</td>';
            echo '</tr>';

            echo '<td><b>' . __('Net_profit') . '</b></td>';
            echo '<td>'. __('total') . '</td>';
            echo '<td><b>' . $this->Number->formatAsCurrency($totalNetProfit) . '</b></td>';
            echo '</tr><tr>';
            echo '<td></td>';
            echo '<td>'. __('per_month') . '</td>';
            echo '<td>' . $this->Number->formatAsCurrency($averageNetProfit) . '</td>';
            echo '</tr><tr>';
            echo '<td><b>' . __('Surcharge') . '</b></td>';
            echo '<td></td>';
            echo '<td><b>' . $this->Number->formatAsPercent($averageSurcharge) . '</b></td>';
            echo '</tr>';
