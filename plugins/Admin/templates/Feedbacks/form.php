...
<div class="filter-container">
    ...
    <div class="right">
        ...
        <?php echo $this->element('headerIcons', ['helperLink' => $this->Html->getDocsUrl(__('docs_route_user_feedback'))]); ?>
    </div>
</div>
...
if (isset($feedback->approved)) {
    ...
    if (!$approved) {
        $approvalWarning = __('Your_feedback_has_not_yet_been_reviewed_by_an_admin_and_is_therefore_not_yet_published.');
        echo '<h2 class="warning" style="margin-bottom:10px;">'.$approvalWarning.'</h2>';
    } else {
        $approvalInfo = __('Your_feedback_has_been_reviewed_and_is_published_on_this_site_{0}._Thank_you.', [
...
if (!$isEditMode || (isset($feedback->approved) && !$approved)) {
    $approvalInfoText = '<p>' . __('Approval_info_text_{0}.', [
        $this->Slug->getFeedbackList(),
    ]) . '</p>';
}

$feedbackIntroText = __('Feedback_intro_text_customer_{0}.', [
    '<b>' . Configure::read('appDb.FCS_APP_NAME') . '</b>',
]);
if ($isManufacturer) {
    $feedbackIntroText = __('Feedback_intro_text_manufacturer_{0}.', [
        '<b>' . Configure::read('appDb.FCS_APP_NAME') . '</b>',
    ]);
}
...
$maxChars = 1000;
$feedbackFieldExplanationText = __('Feedback_field_explanation_text_customer.');
if ($isManufacturer) {
    $feedbackFieldExplanationText = __('Feedback_field_explanation_text_manufacturer.');
}
echo $this->Form->control('Feedbacks.text', [
    'label' => '<span class="small">'. $feedbackFieldExplanationText . '<br /><br /><i>' . __('Max._{0}_characters.', [
        $this->Number->formatAsDecimal($maxChars, 0),
    ]) . '</i>',
    'type' => 'textarea',
    'placeholder' => __('Please_enter_your_feedback_here.'),
    'maxlength' => $maxChars,
    'escape' => false,
]);

echo $this->Form->control('Feedbacks.privacy_type', [
    'label' => __('Show_name?') .' <span class="after small">'.__('Privacy_type_explanatin_text.').'</span>',
...
if (isset($feedback->approved_checkbox) && $identity->isSuperadmin()) {
    echo $this->Form->control('Feedbacks.approved_checkbox', [
        'label' => __('Approved') . ' <span class="after small">'.__('Only_superadmins_can_approve_feedbacks.').'</span>',
        'type' => 'checkbox',
        'escape' => false,
    ]);
}
