...
foreach ($orderDetails as $orderDetail) {
    ...
    if ($groupBy == '') {
        ...
        if (!empty($orderDetail->timebased_currency_order_detail)) {
            echo '<b class="timebased-currency-time-element" title="Zusätzlich in '.Configure::read('appDb.FCS_TIMEBASED_CURRENCY_NAME'). ': ' . $this->Time->formatDecimalToHoursAndMinutes($orderDetail->timebased_currency_order_detail->time).'">&nbsp;*</b>';
        }
    } else {
